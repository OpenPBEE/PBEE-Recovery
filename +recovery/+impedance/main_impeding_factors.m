...
sys_repair_trigger.flooding = zeros(num_reals, 1);
for sys = 1:num_sys
    ...
    for tu = 1:length(damage.tenant_units)
        ...
        sys_repair_trigger.rapid_permit(:,sys) = max( ...
            ...
            max(is_damaged .* (sys_filt & damage.fnc_filters.permit_rapid), [], 2) ...
        );
        ...
        sys_repair_trigger.full_permit(:,sys) = max( ...
            ...
            max(is_damaged .* (sys_filt & damage.fnc_filters.permit_full), [], 2) ...
        );
    
        % Track if any systems require redesign
        sys_repair_trigger.redesign(:,sys) = max( ...
            sys_repair_trigger.redesign(:,sys), ...
            max(is_damaged .* (sys_filt & damage.fnc_filters.redesign), [], 2) ...
        );
    
        % Track if any damage exists that triggers flooding
        sys_repair_trigger.flooding = max( ...
            sys_repair_trigger.flooding, ...
            max(is_damaged .* (sys_filt & damage.fnc_filters.causes_flooding), [], 2) ...
        );
    end
end
...
%% Simulate impeding factors and temp repair that occur in parallel with temp repair scheudl
